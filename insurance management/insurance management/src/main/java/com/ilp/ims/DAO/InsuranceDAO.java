...
public class InsuranceDAO {
	...
	public int registerHealthInsurancePolicy(HealthInsurance healthInsurance)
		...
		try {
			...
			pst1 = conn.prepareStatement(
					"INSERT INTO INSURANCE VALUES(POLICY_REFERENCE_NUMBER_SEQ.NEXTVAL,?,?,?,?,?,?,?,?,'ACTIVE','HEALTH')");
			pst1.setInt(1, healthInsurance.getCustomerId());
			pst1.setInt(2, healthInsurance.getPolicyId());
			pst1.setDate(3, new java.sql.Date(healthInsurance.getPolicyCommencementDate().getTime()));
			pst1.setDate(4, new java.sql.Date(healthInsurance.getPolicyEndDate().getTime()));
			...
			conn.commit();
			if (x > 0) {
				pst2 = conn.prepareStatement("Select POLICY_REFERENCE_NUMBER_SEQ.CURRVAL from dual");
	...
	public int registerVehicleInsurancePolicy(VehicleInsurance vehicleInsurance)
		...
		try {
			...
			conn = DBConnection.getConnection();
			pst1 = conn.prepareStatement(
					"INSERT INTO INSURANCE(Customer_Id,Policy_Id,Policy_commencement_Date,Policy_End_Date,Payment_Mode,Payment_Frequency,Policy_Tenure,Policy_Premium_Amount,Policy_Status,Policy_type) VALUES(?,?,?,?,?,?,1,?,'ACTIVE',?)");
			pst1.setInt(1, vehicleInsurance.getCustomerId());
			pst1.setInt(2, vehicleInsurance.getPolicyId());
			pst1.setDate(3, new java.sql.Date(vehicleInsurance.getPolicyCommencementDate().getTime()));
			pst1.setDate(4, new java.sql.Date(vehicleInsurance.getPolicyEndDate().getTime()));
			...
			pst1.setString(8, "Vehicle");
			x = pst1.executeUpdate();
			if (x > 0) {
				pst2 = conn.prepareStatement(
						"INSERT INTO VEHICLE_INSURANCE(Vehicle_No,License_No,Vehicle_Type) VALUES(?,?,?) returning POLICY_REF_No");
				...
				// if (y > 0) {
				// pst3 = conn.prepareStatement("Select POLICY_REFERENCE_NUMBER_SEQ.CURRVAL from DUAL");
	...
	// Update policy
	public VehicleInsurance updatePolicyDetails(int policyReferenceNumber, String paymentMode, String paymentFrequency)
			throws Exception {
		...
		try {
			...
			conn = DBConnection.getConnection();
			pst1 = conn.prepareStatement(
					"UPDATE INSURANCE SET PAYMENT_MODE=?," + "PAYMENT_FREQUENCY=? WHERE POLICY_REFERENCE_NUMBER=?");
	...
	public VehicleInsurance viewPolicy(int policyReferenceNumber) throws SQLException {
	...
	public ArrayList<VehicleInsurance> viewPoliciesToDelete(int customerId, String otype) throws SQLException {
		...
		try {
			...
			if (otype.equalsIgnoreCase("Insurance officer")) {
				query = "SELECT INSURANCE.POLICY_REF_No,INSURANCE.CUSTOMER_ID,INSURANCE.POLICY_ID, INSURANCE.POLICY_COMMENCEMENT_DATE, INSURANCE.POLICY_END_DATE, "
						+ "INSURANCE.PAYMENT_MODE, INSURANCE.PAYMENT_FREQUENCY, INSURANCE.POLICY_TENURE, INSURANCE.POLICY_PREMIUM_AMOUNT, INSURANCE.POLICY_STATUS, INSURANCE.POLICY_TYPE,INSURANCE.VEHICLE_NO, "
						+ "INSURANCE.LICENSE_NO,INSURANCE.VEHICLE_TYPE FROM INSURANCE LEFT JOIN VEHICLE_INSURANCE ON "
						+ "VEHICLE_INSURANCE.POLICY_REF_No = INSURANCE.POLICY_REF_No WHERE INSURANCE.CUSTOMER_ID=? AND INSURANCE.POLICY_TYPE='HEALTH'";
			}
			if (otype.equalsIgnoreCase("Field officer")) {
				query = "SELECT INSURANCE.POLICY_REF_No,INSURANCE.CUSTOMER_ID,INSURANCE.POLICY_ID, INSURANCE.POLICY_COMMENCEMENT_DATE, INSURANCE.POLICY_END_DATE, "
						+ "INSURANCE.PAYMENT_MODE, INSURANCE.PAYMENT_FREQUENCY, INSURANCE.POLICY_TENURE, INSURANCE.POLICY_PREMIUM_AMOUNT, INSURANCE.POLICY_STATUS,VEHICLE_NO, "
						+ "LICENSE_NO,VEHICLE_TYPE FROM INSURANCE LEFT JOIN VEHICLE_INSURANCE ON "
						+ "VEHICLE_INSURANCE.POLICY_REF_No = INSURANCE.POLICY_REF_No WHERE INSURANCE.CUSTOMER_ID=?";
			}
			pst1 = conn.prepareStatement(query);
			...
			while (rs.next()) {
				...
				insurance.setPremiumAmount(rs.getDouble(9));
				insurance.setPolicyStatus(rs.getString(10));
				insurance.setVehicleNo(rs.getString(11));
				insurance.setLicenseNo(rs.getString(12));
				insurance.setVehicleType(rs.getString(13));
				list.add(insurance);
			}
	...
	public boolean[] terminatePolicy(String[] policyIdlist) throws Exception {
		...
		try {
			...
			for (String ListOfPolicies : policyIdlist) {
				...
				if (vehicleInsurance.getVehicleNo() == null || vehicleInsurance.getVehicleNo() == "")
					vehicleInsurance.setVehicleNo("NA");
				if (vehicleInsurance.getLicenseNo() == null || vehicleInsurance.getLicenseNo() == "")
					vehicleInsurance.setLicenseNo("NA");
				if (vehicleInsurance.getVehicleType() == null || vehicleInsurance.getVehicleType() == "")
					vehicleInsurance.setVehicleType("NA");
				long diff = 0;

				long diffDays = diff / (24 * 60 * 60 * 1000);
				if (diffDays < 15) {
					pst2 = conn.prepareStatement("DELETE FROM INSURANCE WHERE POLICY_REF_No=?");
					pst2.setInt(1, vehicleInsurance.getPolicyReferenceNumber());
					int i = pst2.executeUpdate();
					// conn.commit();
					// if (i > 0) {
					// pst1 = conn.prepareStatement(
					// "INSERT INTO INSURANCE_HISTORY VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
					// pst1.setInt(1, vehicleInsurance.getPolicyReferenceNumber());
					// pst1.setInt(2, vehicleInsurance.getPolicyId());
					// pst1.setInt(3, vehicleInsurance.getCustomerId());
					// pst1.setDate(4, new java.sql.Date(vehicleInsurance.getPolicyCommencementDate().getTime()));
					// pst1.setDate(5, new java.sql.Date(vehicleInsurance.getPolicyEndDate().getTime()));
					// pst1.setString(6, vehicleInsurance.getPaymentMode());
					// pst1.setString(7, vehicleInsurance.getPaymentFrequency());
					...
					// pst1.setDate(12, new java.sql.Date((new Date()).getTime()));
	...
	public void makePolicyInactive() throws Exception {
		try {
			conn = DBConnection.getConnection();
			pst1 = conn.prepareStatement(
					"UPDATE insurance SET POLICY_STATUS = 'INACTIVE' WHERE POLICY_END_DATE >= CURRENT_DATE;");
			pst1.executeUpdate();
		} finally {
			DBConnection.closeStatement(pst1);
			DBConnection.closeConnection(conn);
	...
	public boolean checkPolicyId(int custId, int policyId) throws Exception {
		...
		try {
			...
			conn = DBConnection.getConnection();
			pst1 = conn.prepareStatement(
					"SELECT POLICY_REF_No FROM INSURANCE WHERE CUSTOMER_ID=? AND POLICY_ID=? AND POLICY_STATUS='ACTIVE'");
