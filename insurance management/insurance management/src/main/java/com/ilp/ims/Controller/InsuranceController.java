...
public class InsuranceController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equalsIgnoreCase("addHealthPolicy")) {
			...
			if (flagValidate) {
				RequestDispatcher rd = request.getRequestDispatcher("addHealthPolicy.jsp");
			...
			try {
				...
				exists = idao.checkPolicyId(Integer.parseInt(request.getParameter("custid")),
				...
				if (exists == false && type != null && type.equalsIgnoreCase("health")) {
					...
					if (checkCustId == true) // customer already exists
					{
						...
						startDate = format.parse(request.getParameter("psdate"));
						endDate = new Date((startDate.getYear() + Integer.parseInt(request.getParameter("tenure"))),
						...
						request.setAttribute("check", i);// FOR ERROR PAGE || FOR ADD HEALTHPOLICY PAGE || FOR
															// CONFIRMATION PAGE
					} else
				...
				if (i > 0) {
					...
					request.setAttribute("page", "healthpolicy"); // COMING FROM THIS(healthpolicy) PAGE IN CONFIRMTION
																	// PAGE
		...
		// ------------------view policies for a customer-------------------//
			...
			if (checkCustId == true) {
				try {
					insurancelist = insuranceService.viewPoliciesToDelete(custId,
							request.getSession(false).getAttribute("type").toString());
		...
		// ----------------------------- update policy ---------------------------//
			...
			String paymentFrequency = request.getParameter("paymentfrequency");
			try {
				vinsurance = insuranceService.updatePolicyDetails(policyReferenceNumber1, paymentMode,
						paymentFrequency);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		// ------------------------------ terminate policy ------------------------------//
			...
			try {
				policyTermination = insuranceService.terminatePolicy(policyIdList);
			} catch (Exception e) {
				e.printStackTrace();
			}
