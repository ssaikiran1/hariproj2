...
public class ClaimDAO {
	...
	public static String getPolicyType(int pref) throws ClassNotFoundException, SQLException {
		...
		ps = con.prepareStatement("SELECT POLICY_TYPE FROM INSURANCE WHERE policy_ref_no=?");
		ps.setInt(1, pref);

		ResultSet rs = ps.executeQuery();
		if (rs.next()) {
			type = rs.getString("POLICY_TYPE");
		}
	...
	public static int submissionOfInsuranceClaim(Claim c) throws ClassNotFoundException, SQLException {
		...
		ps = con.prepareStatement(
				"INSERT INTO CLAIM_DETAILS(Policy_Ref_No,Claim_Amount,Claim_submission_Date,Beneficiary_Name,Beneficiary_Address,Beneficiary_Email,Beneficiary_Contact_no,Claim_Status,Claim_Rejection_Reason) VALUES(?,?,?,?,?,?,?,?,?) returning claim_id");
	...
	public static boolean checkPolicyRefNo(int pref) throws SQLException, ClassNotFoundException {
		...
		Connection con = null;
		con = DBConnection.getConnection();
		ps = con.prepareStatement("SELECT * FROM INSURANCE WHERE policy_ref_no=? AND POLICY_STATUS='ACTIVE'");
		ps.setInt(1, pref);
		ResultSet rs = ps.executeQuery();
	...
	public static boolean checkExistclaim(int pref) throws ClassNotFoundException, SQLException {
		...
		ps = con.prepareStatement("SELECT * FROM CLAIM_DETAILS WHERE policy_ref_no=? AND CLAIM_STATUS='open'");
	...
	public static Claim getClimDetails(int claimId) throws ClassNotFoundException, SQLException {
		...
		if (rs.next()) {
			...
			c.setPolicyReferenceId(rs.getInt(3));
			c.setClaimAmount(rs.getDouble(4));
			c.setClaimSubmissionDate(rs.getDate(5));
			c.setBeneficiaryName(rs.getString(6));
			c.setBeneficiaryAddress(rs.getString(7));
			c.setBeneficiaryEmail(rs.getString(8));
			c.setBeneficiaryContactNo(rs.getLong(9));
		} else {
	...
	public static String getPolicyStatusClaimId(int claimId) throws SQLException, ClassNotFoundException {
		...
		ps = con.prepareStatement(
				"SELECT INSURANCE.POLICY_TYPE AS insurance_policy_type FROM INSURANCE INNER JOIN CLAIM_DETAILS ON INSURANCE.policy_ref_no = CLAIM_DETAILS.policy_ref_no WHERE CLAIM_ID = ?");
		ps.setInt(1, claimId);
	...
	public static boolean updateClaimDetails(Claim claim) throws ClassNotFoundException, SQLException {
		...
		ps = con.prepareStatement(
				"UPDATE CLAIM_DETAILS SET CLAIM_AMOUNT = ?,BENEFICIARY_NAME = ?,BENEFICIARY_ADDRESS = ?,BENEFICIARY_EMAIL = ?,BENEFICIARY_CONTACT_NO = ? WHERE CLAIM_ID=?");
	...
	public static boolean approvalClaim(int claimId) throws ClassNotFoundException, SQLException {
		...
		ps = con.prepareStatement(
				"UPDATE CLAIM_DETAILS SET CLAIM_STATUS = ?,CLAIM_REJECTION_REASON='' WHERE CLAIM_ID=?");
