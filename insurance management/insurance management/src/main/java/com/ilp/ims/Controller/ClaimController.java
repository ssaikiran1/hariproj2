...
public class ClaimController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("addClaim")) {
			...
			try {
				if (cs.checkExistclaimService(Integer.parseInt(request.getParameter("prefno")))) {
					if (cs.checkPolicyRefNoService(Integer.parseInt(request.getParameter("prefno")))) // ---->check
																										// policy
																										// reference
																										// number in
																										// database
					{
						...
						if ((type.equalsIgnoreCase("health") && utype.equalsIgnoreCase("INSURANCE OFFICER"))
								|| (type.equalsIgnoreCase("VEHICLE") && utype.equalsIgnoreCase("FIELD OFFICER"))) {
							...
							out.print("<script>if(window.confirm('" + utype + " can not add " + type + " claim'))"
									...
									+ "else{window.location = 'home.jsp';" + "}" + "</script>");
						}
					} else {
						// out.print("Policy reference number does not exist");
						out.print(
								"<script>if(window.confirm('Policy reference number does not exist or Policy is not active.'))"
										+ "{window.location = 'applyClaim.jsp';" + "}"
										+ "else{window.location = 'home.jsp';" + "}" + "</script>");
					}
				} else {
					// out.print("Policy reference number does not exist");
					out.print("<script>if(window.confirm('Claim for Policy reference Number :"
							+ request.getParameter("prefno") + "exist and is Open. '))"
							+ "{window.location = 'applyClaim.jsp';" + "}" + "else{window.location = 'home.jsp';" + "}"
							+ "</script>");
				}
		...
		// ------->first screen for update page, it will "check claim id exist?","check officer type","check policy
		// status"
		...
		if (action.equals("updateDetail")) {
			...
			try {
				claimFlag = cs.checkClaimIdService(Integer.parseInt(request.getParameter("claimid"))); // check claim id
																										// exist or not
			...
			{
				try {
					type = cs.getPolicyStatusClaimId(Integer.parseInt(request.getParameter("claimid"))); // get policy
																											// status by
																											// claim id
				} catch (NumberFormatException e1) {
					e1.printStackTrace();
				...
				if ((type.equalsIgnoreCase("health") && utype.equalsIgnoreCase("INSURANCE OFFICER"))
					...
					out.print("<script>if(window.confirm('" + utype + " can not update " + type + " claim'))"
							+ "{window.location = 'updateClaim.jsp';" + "}" + "else{window.location = 'home.jsp';" + "}"
							+ "</script>");
				}
			} else {
				...
				if ((type.equalsIgnoreCase("health") && utype.equalsIgnoreCase("INSURANCE OFFICER"))
					...
					try {
						if (cs.checkClaimStatusService(claimId)) {
							...
							if (s)
								out.print("<script>if(window.confirm(' " + request.getParameter("claimid")
										+ " Deleted'))" + "{window.location = 'cancleClaim.jsp';" + "}"
										+ "else{window.location = 'home.jsp';" + "}" + "</script>");
							else
								out.print("<script>if(window.confirm(' " + request.getParameter("claimid")
										+ " not Deleted'))" + "{window.location = 'cancleClaim.jsp';" + "}"
										+ "else{window.location = 'home.jsp';" + "}" + "</script>");
						} else {
							out.print("<script>if(window.confirm('Claim status is not open'))"
					...
					out.print("<script>if(window.confirm('" + utype + " can not cancel " + type + " claim'))"
							+ "{window.location = 'cancleClaim.jsp';" + "}" + "else{window.location = 'home.jsp';" + "}"
							+ "</script>");
				}
			} else {
				out.print("<script>if(window.confirm('Claim Id does not exist.'))"
		...
		if (action.equals("ArClaimDetails")) {
			...
			if (claimFlag) {
				...
				String utype = request.getSession(false).getAttribute("type").toString();
				if (utype.equalsIgnoreCase("INSURANCE OFFICER")) {
					Claim c = null;
					...
					out.print("<script>if(window.confirm('Only admin can approve/reject claim'))"
							+ "{window.location = 'approveClaim.jsp';" + "}" + "else{window.location = 'home.jsp';"
							+ "}" + "</script>");
				}
			} else {
				out.print("<script>if(window.confirm('Claim Id does not exist.'))"
		...
		if (action.equalsIgnoreCase("Open") || action.equalsIgnoreCase("Approved")
			...
			if (utype.equalsIgnoreCase("INSURANCE OFFICER")) {
				...
				out.print("<script>if(window.confirm('Only admin can View claim'))"
						+ "{window.location = 'ViewClaim.jsp';" + "}" + "else{window.location = 'home.jsp';" + "}"
						+ "</script>");
			}
