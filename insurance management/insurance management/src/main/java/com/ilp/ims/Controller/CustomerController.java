...
public class CustomerController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("addCustomer")) {
			...
			String addressProofid = request.getParameter("addressproofid");
			...
			if (addressProofid.equals("default")) {
			...
			customer.setAddressproofid(addressProofid);
			...
			try {
				...
				if (ID > 0) {
					...
					out.print("<script>if(window.confirm('Email already exist.'))"
							+ "{window.location = 'addCustomer.jsp';}" + "else{window.location = 'home.jsp';}"
							+ "</script>");
				}
		...
		// -------------------------getting all data to show from textboxes ------------------------------
			...
			try {
				...
				if (customerService.checkCustomerId(Integer.parseInt(custId))) {
					...
					out.print("<script>if(window.confirm('Customer with ID: " + request.getParameter("custid")
							+ " does not exist.'))" + "{window.location = 'updateCustomer.jsp';}"
			...
			try {
				...
				if (customerService.updateCustomerDetails(customer)) {
					...
					out.print("<script>if(window.confirm('Details of " + customerName + " updated successful.'))"
							+ "{window.location = 'home.jsp';}" + "else{window.location = 'home.jsp';}" + "</script>");

				} else {
					out.print("<script>if(window.confirm('Error in updation.'))" + "{window.location = 'home.jsp';}"
							+ "else{window.location = 'home.jsp';}" + "</script>");
				}
			} catch (Exception ex) {
			...
			try {
				...
				if (custList.size() > 0) {
					...
					out.print("<script>if(window.confirm('No customers between given registration dates!!!'))"
							+ "{window.location = 'viewCustomer.jsp';}" + "else{window.location = 'home.jsp';}"
							+ "</script>");
				}
			} catch (Exception ex) {
				System.out.println(ex.getMessage());
				...
				if (customerService.checkCustomerId(Integer.parseInt(custId))) {
					if (customerService.checkPolicyStatus(Integer.parseInt(custId))) {
						if (customerService.removeCustomer(Integer.parseInt(custId))) {
							out.print("<script>if(window.confirm('Customer with ID: " + request.getParameter("custid")
									+ " deleted.'))" + "{window.location = 'home.jsp';}"
									+ "else{window.location = 'home.jsp';}" + "</script>");
						}
					} else {
						out.print("<script>if(window.confirm('Customer with ID: " + request.getParameter("custid")
								+ " may have an ACTIVE POLICY.'))" + "{window.location = 'removeCustomer.jsp';}"
								+ "else{window.location = 'home.jsp';}" + "</script>");
					}
				} else {
					out.print("<script>if(window.confirm('Customer with ID: " + request.getParameter("custid")
							+ " does not exist.'))" + "{window.location = 'removeCustomer.jsp';}"
							+ "else{window.location = 'home.jsp';}" + "</script>");
				}
			} catch (SQLException e) {
				e.printStackTrace();
